        ...
        <div className='row'>
          ...
          <div className='col-md-12 col-lg-4 mt-5 ps-md-5'>
            <div className='row'>
              ...
              <div className='col-12'>
                {
                  postData?.home_world_2?.map((homeWorld: {
                    ...
                    <div key={indx.toString()} className="mb-3">
                      ...
                      {homeWorld?.image?.url ?
                        <img
                          ...
                          src={homeWorld?.image?.url + '?width=100'}
                ...
                {postData?.powers ? <p {...postData.$?.powers as {}}><strong>Power{postData?.powers.length > 1 ? 's' : ''} :</strong> {postData?.powers.join(', ')}</p> : ''}
              </div>
            </div>
          </div>
        </div>
...
export async function getServerSideProps({ params }: any) {
  try {
    const posts = await getComposableHeroSingleRes(`/composable-heroes-2/${params.post}`);
    if (!posts) throw new Error('404');

    return {
      props: {
        pageUrl: `/composable-heroes-2/${params.post}`,
        superHeroPost: posts,
      },
    };
  } catch (error) {
